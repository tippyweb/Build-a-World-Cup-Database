...
######################################################################################
# Worldcup Database Queries - 2024-09-20 - All tests passed - Submitting on 2024-09-21
######################################################################################
...
#echo "$($PSQL "SELECT t1.name, t2.name FROM games 
#               LEFT JOIN teams AS t1 ON games.winner_id = t1.team_id 
#               LEFT JOIN teams AS t2 ON games.opponent_id = t2.team_id
#               WHERE year = 2014 AND round = 'Eighth-Final'
#     ")"

echo "$($PSQL "SELECT teams.name FROM games 
               LEFT JOIN teams ON games.winner_id = teams.team_id 
               WHERE year = 2014 AND round = 'Eighth-Final'
               UNION ALL
               SELECT teams.name FROM games
               LEFT JOIN teams ON games.opponent_id = teams.team_id
               WHERE year = 2014 AND round = 'Eighth-Final'
               ORDER BY name
     ")"
